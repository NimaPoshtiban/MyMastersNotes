...
\end{enumerate}
	...
	\question Eloquently elaborate the mechanism of the following Adder/Subtractor
	\begin{figure}[H]
		\includegraphics{2scompl.jpg}
	\end{figure}
	\begin{solution}
		The depicted logical circuit removes the disadvantage of asymmetric range.\\ If the operand y is\textbf{ $-2^{k-1}$}, represented in 2’s complement as 1
		followed by all $0s$, its complementation does not lead to overflow. This is because the
		2’s complement of y is essentially represented in two parts: ycompl, which represents
	\textbf{	$2^{k-1} -1$}, and \textbf{cin} which represents \textbf{1}.
	For summation of 8bit and 16bit numbers the complexity \textit{$M=2^{k}$} goes to $M\acute{}\,=2^{k\acute{}}$ thus, \[
		M\acute{} - M \,=\,2^{k}\left( 2^{k\acute{}-k}-1 \right) \newline
		\Longrightarrow 2^{8}(2^{16-8}-1)
	\]
	Because the sign extension creates these extra operations
	\end{solution}
\end{questions}

\begin{figure}[H]
	\includegraphics{equ1.pdf}
\end{figure}
